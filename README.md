...
powershell -ExecutionPolicy Bypass -File .\install-car.ps1 `
  -ApkPath .\road-assistant-2.11.apk
```
...
- Приложение читает сигналы VHAL. Оно не записывает свойства автомобиля.
  Единственная команда IPKService переключает навигационную область между
  режимами `MINI` и `RESET`.
