...
powershell -ExecutionPolicy Bypass -File .\install-car.ps1 `
  -ApkPath .\RoadAssistant-2.52-instrument-tbt-minimap-hud.apk
```
...
1. [FakeTeleNav](https://github.com/DmitryRND/exeed-fake-telenav) — штатный
   TeleNav/IPK/HUD-адаптер и поверхность карты Launcher.
...
- Мини-карта антирадара требует Android 8/API 26+, интернет, GPS и API-ключ
  MapKit. Она загружает актуальные тайлы Яндекса самостоятельно и показывает
  камеры Дорожного ассистента, но пока не рисует полилинию маршрута Навигатора.
  Полноэкранный режим на приборке запускает непосредственно новый Навигатор.
